...
inline void doubleToArr(double ht,AppCMsg *msg)
{
	nx_int8_t *p=(nx_int8_t *)&ht;
	msg->doubleArr0=p[0];
 	msg->doubleArr1=p[1];
 	msg->doubleArr2=p[2];
 	msg->doubleArr3=p[3];
...
inline double arrToDouble(AppCMsg *msg)
{
	double temp;
	nx_int8_t *p=(nx_int8_t *)&temp;
	*p=msg->doubleArr0;
 	*(p+1)=msg->doubleArr1;
 	*(p+2)=msg->doubleArr2;
 	*(p+3)=msg->doubleArr3;
...
//The corutine main function
  event void Co0.run()
  {
   ...
   {	
	...
	while (TRUE){
	mode=1;
	receiveMsg.msg_counter=-1;
	receiveMsg.number=100;
	case 1:
	...
	doubleToArr(arrToDouble(&receiveMsg)+3,&sendMsg);
